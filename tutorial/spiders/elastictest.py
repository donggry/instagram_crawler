...
count=0
start_time = testtimecrawl.time()
print start_time
three_month = 60 * 60 * 24 * 90
print three_month
a=1466947483-1464877867
print a/(60*60*24)
print testtimecrawl.gmtime(1464877867)
...
class InstagramSpider(scrapy.Spider):
    name = "instagram_search_trend0"  #내가 정할 크롤러 이름
    ...
    def parse_page(self, response):
        ...
        for posts in media:
            ...
            """scoring 하는 부분"""
            score_time = (self.start_time - item["date"]) / (60*60*24*2) # 2일
            score_likes = 0
            if ("likes" in posts):
            ...
            score = (score_likes + score_comments) / score_time
            print item['posts'].encode('utf-8')
            global count
            """ES삽입부"""
            """self.es.index(index="instagram", doc_type="uncategorizedinstagram",
                          body={"contents": item["posts"], "likes": item["likes"], "date": item["date"], "comments_count" : item["comments_count"],
                                "score": score})"""
            for word in words:
                if (word in self.category):
                    cat_word = word.decode('utf-8')
                    self.es.index(index="scoretest2", doc_type="categorizedinsta",id=cat_word+item['id'].encode('utf-8'),
